<template>
  <div class="calendar-container">
    ...
    <div class="weeks align-center">
      <p v-for="week in weeks" :key="week" class="week">
        {{ week }}
      </p>
    </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const dateFactory = (year: number, month: number): Array<Date> => {
      ...
      const startDate = new Date(
        monthStartDate.setDate(
          monthStartDate.getDate() - monthStartDate.getDay()
        )
      );
      console.log(startDate);
      const endDate = new Date(
        monthLastDate.setDate(
    ...
    const dates: Ref<Array<Date>> = ref(
      dateFactory(new Date().getFullYear(), new Date().getMonth())
    );
